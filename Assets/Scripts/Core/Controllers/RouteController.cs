...
public class RouteController : BaseUIPopUp
{
    public event TrainAction<Vector2> TrainSpawn;
    public delegate TrainView TrainAction<T>(T component);
    ...
    private List<CityView> _cities = new();
    private RouteScheme _route = new();
    private TrainView _train;
    ...
    private bool CheckWaysOnActivity()
    {
        for(int i = 0; i < _cities.Count - 1; i++)
        {
            ...
            if(WayService.TryGetRail(cityFrom, cityTo, out RailView rail))
            {
                if(rail.gameObject.activeSelf) continue;
                else return false;
            }
            else if (WayService.TryGetBridge(cityFrom, cityTo, out BridgeView bridge))
            {
