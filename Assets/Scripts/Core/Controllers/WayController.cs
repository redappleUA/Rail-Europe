...
public class WayController : BaseUIPopUp
{
    ...
    private readonly List<ClickableObjectView> _clickableObjects = new();
    ...
    private void GetCityFromForWay(ClickableObjectView clickableObject)
    {
        // Set the starting city for the way
        CityFrom = CityService.GetCityFromObject(clickableObject);
    }

    private void AddClickableObjects(ClickableObjectView clickableObject)
    {
    ...
    private void TurnOnWay(ClickableObjectView clickableObject)
    {
        ...
        (RailView rail, BridgeView bridge) ways = (rail: null, bridge: null);
